...
public class GameShow extends JPanel implements Runnable,MouseListener,KeyListener{
	...
	public void paintComponent(Graphics g) 
	{
		...
		g.drawString("Click on a vehicle to switch types",500,150);
		...
		if(lose)
		{
			...
			g.drawString("You passed "+c+" vehicles",475,350);
		}
	}
	...
	public void mousePressed(MouseEvent e)
	{
		for(int i=0;i<arr.size();i++)
		{
			if(arr.get(i).contains(e.getX(),e.getY()))
			{
				if(arr.get(i).getId().equals("car"))
				{
					arr.set(i,new Truck(arr.get(i).getX(),arr.get(i).getY(),"",getRandomColor(),-speed));
				}
				else
				{
					arr.set(i,new Car(arr.get(i).getX(),arr.get(i).getY(),"",getRandomColor(),-speed));
				}
			}
		}
		repaint();
